...
import { NgbModal, NgbDateStruct, NgbCalendar, NgbDatepickerConfig, NgbModalRef } from '@ng-bootstrap/ng-bootstrap';
...
export class BookingComponent implements OnInit {
  ...
  constructor(private calendar: NgbCalendar, config: NgbDatepickerConfig, private staffService: StaffService,
    ...
    private tostr: ToastrService, private modalService: NgbModal) {
  ...
  onSubmit(bookingForm: NgForm) {
    ...
    if (this.checkValidTimeBook) {
      ...
      this.tostr.success('Đặt thành công', 'Đặt lịch', {
        timeOut: 1000,
        progressBar: true
      });
      this.modalRef.close();
  ...
  open(contentBooking) {
    this.modalRef = this.modalService.open(contentBooking, { size: 'lg' });
  }
